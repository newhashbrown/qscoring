/**
 * Policy exposure tagging — offline generation (AI-analysis roadmap, Phase 3).
 *
 * Clones the narratives pipeline INFRA (scripts/generate-narratives.ts): pull
 * grounding from the Worker, skip unchanged by input_hash, generate via the
 * Anthropic Message Batches API (batch-only cost guardrail, forced tool), one
 * corrective batch for parse failures, persist survivors. What DIFFERS: the
 * grounding is FMP-profile-derived (sector/industry/description), there is no
 * score band, and the numeric guardrail is replaced by the DEGENERACY check —
 * every number is meaningless here; the real failure mode is a classifier that
 * rubber-stamps all six tags the same, which degenerateReason rejects.
 *
 * Run:
 *   ANTHROPIC_API_KEY=… SNAPSHOT_CRON_TOKEN=… npm run policy-tags -- --sample
 *   npm run policy-tags -- --limit 50
 *   npm run policy-tags -- --dry-run --sample     # print a payload, no API calls
 * Env: POLICY_MAX_TICKERS caps tickers per run; POLICY_MODEL overrides the model.
 */
...
import type { PolicyPayload } from "../lib/policy/grounding";

const BASE = process.env.QSCORING_BASE ?? "https://qscoring.com";
// Same fast, batch-supported model as narratives. Batch pricing $0.50/$2.50 per MTok.
...
const SAMPLE_TICKERS = ["AAPL", "PFE", "XOM", "GOOGL", "WMT"]; // spans tech/pharma/energy/antitrust/retail
...
async function fetchGrounding(tickers: string[], token: string): Promise<Grounding[]> {
  ...
  for (const group of chunk(tickers, GROUNDING_CHUNK)) {
    const res = await fetch(`${BASE}/api/cron/policy-grounding`, {
      method: "POST",
      headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
      body: JSON.stringify({ tickers: group, promptVersion: POLICY_PROMPT_VERSION }),
    });
...
/** A batch request per grounding, forcing the emit_policy_exposures tool. */
...
type BatchOutput = { byTicker: Map<string, PolicyExposures | null>; usage: Usage };

/** Submit one batch, poll to completion, return parsed tool outputs + usage. */
async function runBatch(client: Anthropic, requests: BatchRequest[]): Promise<BatchOutput> {
...
async function persist(rows: unknown[], token: string): Promise<number> {
  ...
  for (const group of chunk(rows, PERSIST_CHUNK)) {
    const res = await fetch(`${BASE}/api/cron/persist-policy-tags`, {
      method: "POST",
      headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
...
async function main() {
  ...
  const envCap = Number(process.env.POLICY_MAX_TICKERS);
  const cap = Math.min(
    args.limit ?? Infinity,
    Number.isFinite(envCap) && envCap > 0 ? envCap : Infinity
  );
  if (Number.isFinite(cap)) tickers = tickers.slice(0, cap);
  console.log(`Policy tags: ${tickers.length} ticker(s) via ${BASE} (prompt ${POLICY_PROMPT_VERSION}, model ${MODEL})`);
  ...
  if (!apiKey) throw new Error("ANTHROPIC_API_KEY is not set — required to generate classifications");
  ...
  // 4. One corrective batch for parse failures only (degeneracy is terminal).
  ...
  for (const g of toGenerate) {
    ...
    const reason = degenerateReason(exposures);
    if (reason) {
      failedDegenerate++;
      console.warn(`  [${g.ticker}] REJECTED (degenerate): ${reason}`);
      continue;
    }
    rows.push({
      ticker: g.ticker,
  ...
  console.log(
    `\nDone. written=${written} rejected(parse)=${failedParse} rejected(degenerate)=${failedDegenerate} ` +
  ...
  if (written === 0 && toGenerate.length > 0) {
    throw new Error(
      `Generated 0 classifications from ${toGenerate.length} candidate(s) — ` +
        `parse-failed=${failedParse}, degenerate=${failedDegenerate}. See per-ticker logs above.`
    );
  }
}
